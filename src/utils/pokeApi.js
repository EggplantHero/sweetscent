...
export async function getImg(input, shiny) {
  let front = "front_";
  front += shiny;
  if (!input) return;
  const poke = await getPoke(input);
  let spriteUrl =
    poke.data.sprites.versions["generation-v"]["black-white"].animated[front];
  if (!spriteUrl) {
    spriteUrl =
      poke.data.sprites.versions["generation-v"]["black-white"][front];
  }
  if (spriteUrl === null) {
    return "blank";
  }
...
export async function getAllPokes() {
  return await http.get(`${apiUrl}/pokemon/?limit=898`);
}
export async function getAllPokeNames() {
  const { data } = await http.get(`${apiUrl}/pokemon/?limit=898`);
